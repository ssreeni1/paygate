...
export function handleEstimate(pricingCache, spendTracker) {
    return async (input) => {
        ...
        try {
            ...
            for (const call of input.calls) {
                if (!call.endpoint || typeof call.count !== 'number' || call.count < 1) {
                    return errorToMcpContent(invalidInput(`Invalid call entry: endpoint="${call.endpoint}", count=${call.count}`));
                }
